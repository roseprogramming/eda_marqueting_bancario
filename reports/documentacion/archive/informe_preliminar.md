...
El análisis exploratorio profundo incluirá distribuciones univariadas (análisis de una sola variable para entender su comportamiento individual) mediante histogramas y boxplots, frecuencias de variables categóricas y detección de outliers (valores atípicos que se alejan significativamente del resto de los datos) por IQR (rango intercuartílico).

El análisis bivariado (análisis de la relación entre dos variables) examinará la relación de variables clave con el target (variable objetivo), tasas de conversión por segmentos y correlaciones numéricas. El análisis multivariado construirá y visualizará la matriz de correlación completa de variables numéricas, identificando posibles casos de multicolinealidad (alta correlación entre dos o más variables independientes que puede distorsionar los resultados del modelo). Además, se realizará un análisis descriptivo de factores de campaña relevantes, como la duración de la llamada y el resultado de campañas previas.
