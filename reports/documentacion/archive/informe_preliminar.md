...
El análisis exploratorio profundo incluirá distribuciones univariadas (análisis de una sola variable para entender su comportamiento individual) mediante histogramas y boxplots, frecuencias de categóricas y detección de outliers (valores atípicos que se alejan significativamente del resto de los datos) por IQR (rango intercuartílico). El análisis bivariado (análisis de la relación entre dos variables) examinará relación de variables con el target (variable objetivo), tasas de conversión por segmentos y correlaciones numéricas. El multivariado construirá matriz de correlación completa, detectará multicolinealidad (alta correlación entre dos o más variables independientes que puede distorsionar los resultados del modelo) mediante VIF (factor de inflación de la varianza) y evaluará pertinencia de PCA (análisis de componentes principales).


El feature engineering (ingeniería de características) creará antiguedad_cliente (date - dt_customer), aplicará binning (agrupamiento en intervalos) en variables continuas si mejora interpretabilidad, generará interacciones relevantes y codificará categóricas mediante one-hot (codificación binaria) o label encoding (codificación ordinal). La preparación para modelado evaluará desbalance de clases (situación en la que una clase está representada con mucha mayor frecuencia que otra, lo que puede afectar el rendimiento del modelo) aplicando SMOTE (técnica de sobremuestreo sintético) o undersampling (submuestreo), dividirá datos en train/validation/test (entrenamiento/validación/prueba), estandarizará variables numéricas y aplicará selección de features (características).
